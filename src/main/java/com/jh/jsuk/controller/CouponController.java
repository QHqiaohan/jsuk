...
public class CouponController {
    ...
    public Result listByUserId(Integer userId) {
        ...
        for (CouponVo coupon : couponList) {
            ...
            if (coupon.getUserCoupon().getStatus() == 1 || coupon.getUserCoupon().getStatus() == 2) {
                coupon.setStatus(coupon.getUserCoupon().getStatus());
    ...
    //用户-我的-优惠券-获取优惠券数量
    @ApiOperation("用户-我的-优惠券-显示优惠券数量")
    @RequestMapping(value="/getCouponNum",method={RequestMethod.GET,RequestMethod.POST})
