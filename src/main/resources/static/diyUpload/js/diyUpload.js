...
(function( $ ) {
    ...
    $.fn.extend({
		...
		*/
        diyUpload:function( opt, serverCallBack ) {
            ...
            if(opt.imgs && opt.preImgUrl){
                initImg(opt.preImgUrl,$fileInput,opt.imgs);
            }
			//迭代出默认配置
			...
			webUploader.on('error', function( code ) {
				...
				switch( code ) {
					...
					break;
					case  'F_EXCEED_SIZE' : text = '单个文件大小超过1M限制!';
					break;
	...
	//用于渲染初始图片显示
	function initImg(preImg,$fileInput,obj) {
        var $parentFileBox = $fileInput.next('.parentFileBox');
		...
		for(var i = 0; i < obj.length; i++){
             li += '<li id="fileBox_'+ i +'" class="diyUploadHover">'+
					'<div class="viewThumb"><img src="'+ preImg +obj[i] +'" alt=""></div>'+
					'<div class="diyCancel"></div>'+
					'<div class="diyFileName">'+ obj[i] +'</div>'+
					'<input type="hidden" class="initImgBox" value="'+ obj[i] +'">'+
				'</li>';
		}

        $parentFileBox.find('.fileBoxUl').html( li );
		//绑定删除事件
        $parentFileBox.find('li .diyCancel').each(function(){
